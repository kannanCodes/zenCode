...
export class CompilerService {
     ...
     async createExecution(input: ExecuteCodeInput): Promise<{ token: string }> {
          ...
          const result = await this.dockerService.executeCode(executionData as any);
          // Store result in memory with a unique token
          const token = `exec_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
          this.executionResults.set(token, result);
          return { token };
     }

     async getExecutionResult(token: string) {
          const result = this.executionResults.get(token);
          if (!result) {
               throw new AppError('Execution result not found', STATUS_CODES.NOT_FOUND);
          }
          // Clean up after retrieval
          this.executionResults.delete(token);
          return result;
     }
}